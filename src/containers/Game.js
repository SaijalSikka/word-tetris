...
import { checkPhrase, sortWordQueue } from '../config/wordCheck';
...
class Game extends Component {
  ...
  moveLetters = () => {
    ...
    for (let i = 0; i < this.letters.length; i++) {
      if (this.letters[i].pos.y < numberOfRow - 1 && this.letters[i].moving) {
        ...
        if (this.letters[i].pos.y === numberOfRow - 1 || alreadyHas) {
          this.letters[i].moving = false;
          this._checkAndCancelRow(this.letters[i].pos.y);
        }
  ...
  _checkPossibleWords = () => {
    this.letters.forEach((_letter, index) => {
      ...
      if (checkPhrase(possibleWord.toLowerCase())) letterEnvolved.forEach(_letter => _letter.isWord = true);
    });
  ...
  _getLetterForThisColumn = (column) => {
    ...
    for (let i = 0; i < this.letters.length; i++) {
      if (this.letters[i].pos.x === column) _letterInColumn.push(this.letters[i]);
    }
  ...
  _checkWordAndDestroy = () => {
    if (this.wordQueue.length > 0) {
      ...
      if (!wordIsInRow) {
        // if not in row, then only check for column
        for (let i = 0; i < this.wordQueue.length - 1; i++) {
      ...
      if (wordIsInRow || wordIsInColumn) {
        ...
        if (checkPhrase(word.toLowerCase())) {
          this._foundValidWord(word, wordIsInRow, wordIsInColumn);
        } else if (checkPhrase(word.toLowerCase().split('').reverse().join(''))) {
  ...
  _checkAndCancelRow = (row) => {
    const fullRow = Array.from({ length: noOfColumn }, (_, i) => i);
    const cancelRow = fullRow.filter(col => !this._alreadyHasLetterInPos({ x: col, y: row }));
